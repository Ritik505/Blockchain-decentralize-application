...
contract LotteryContract {
    ...
    uint8 private availableNumbers = 15; 
    ...
    uint256 public constant WINNER_PRIZE = 0.5 ether; 
    uint256 public constant MANAGER_FEE = 0.1 ether; 
    ...
    function buyTicket() external payable gameInProgress {
        ...
        require(msg.value == TICKET_PRICE, "Ticket price is 0.2 AVAX");
    ...
    function determineWinner() external onlyManager gameInProgress {
        ...
        emit WinnerDeclared(winnerAddress, randomNumber, winnerAddress != address(0) ? WINNER_PRIZE : 0);
    }
    ...
    function distributeWinnings(address winnerAddress) private {
        if (winnerAddress != address(0)) {
            ...
            payable(winnerAddress).transfer(WINNER_PRIZE);
            payable(manager).transfer(MANAGER_FEE);
            
            emit FundsTransferred(winnerAddress, WINNER_PRIZE);
            emit FundsTransferred(manager, MANAGER_FEE);
        } else {
           
            payable(manager).transfer(address(this).balance);
            
            emit FundsTransferred(manager, address(this).balance);
        }
    }
