  ...
  const updateAvailableNumbers = async () => {
    try {
      if (contract && web3 && !isUpdatingNumbers) {
        ...
        const availableNums = numbers
          .map(n => parseInt(n))
          .filter(num => !Object.values(selectedNumbers).includes(num));
  ...
  const buyTicket = async (participantNumber) => {
    try {
      ...
      const result = await contract.methods.buyTicket().send({
        from: account,
        value: web3.utils.toWei('0.2', 'ether') // 0.2 AVAX
      });
  ...
  return (
    <div id="game-section">
      ...
      <div id="ticket-section">
        <h3>Buy Tickets (0.2 AVAX each)</h3>
            ...
            <div className="selection-area">
              ...
              <div className="number-buttons">
                {availableNumbers.map(number => (
                  <button 
                    key={number}
                    className={`number-btn ${selectedNumbers[currentParticipant + 1] === number ? 'selected' : ''}`}
                    onClick={() => selectNumber(number)}
                    disabled={selectedNumbers[currentParticipant + 1] === 'selecting...'}
                  >
                    {number}
                  </button>
                ))}
              </div>
              <p>Selected: <span className="selected-number">
      ...
      {showWinner && (
        <div id="winner-display">
          ...
          <p>Prize: <span id="prize-amount">{winnerInfo.prize}</span> AVAX</p>
        </div>
      )}
    </div>
  );
